...
### `ParquetFileReaderFactory::create_reader` receives pre-registered metrics

Custom `ParquetFileReaderFactory` implementations now receive a
`ParquetMetricSet` containing pre-registered metric handles rather than an
`ExecutionPlanMetricsSet` that requires the factory to register them. This lets
the Parquet scan control metric cardinality before reader creation:
...
The set is shared by all files in an execution partition in compact mode and
is dedicated to one file in verbose mode. Readers should call
`metrics.add_bytes_scanned(bytes)` so `bytes_scanned` and the scan-efficiency
numerator remain consistent.
